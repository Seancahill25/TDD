...
namespace Tests
{
    public class Tests
    {
        ...
        [Test]
        public void ReturnsClientNetWorth()
        {
            var client = new Client
            {
                currentAge = 47,
            ...
            var calculator = new Calculator();
            var output = calculator.RetirementYears(client);
            var expectedOutput = client.netWorth;
            Assert.AreEqual(expectedOutput, output);
        }

        [Test]
        public void ReturnNetWorthOnRetirment()
        {
            var client = new Client
            {
                currentAge = 47,
                ...
                netWorth = 500000,
                desiredMonthlySpending = 2500,
                yearlySavingContribution = 55000,
                yearlyExpenses = 22000
            };
            var calculator = new Calculator();
            var output = calculator.RetirementYears(client);
            var expectedOutput = 1094000;
            Assert.AreEqual(expectedOutput, output);
        }

        [Test]
        public void ReturnYearsFundsWillLast()
        {
            var client = new Client
            {
                currentAge = 47,
